...
def calculate_sobre_c(tender_id: str, body: SobreCCalculateRequest):
    ...
    supplier_names = {s["id"]: s["name"] for s in config["suppliers"]}

    missing = [sid for sid in supplier_names if sid not in body.declared_values]
    if missing:
        raise HTTPException(
            status_code=422,
            detail=f"Missing declared values for suppliers: {missing}",
        )

    try:
        raw = score_sobre_c(
            criteria_def=SOBRE_C_CRITERIA[tender_id],
            declared_values=body.declared_values,

 ...
 * Root Directory is `frontend`; an identical copy lives at the repo root for
 * the case where the Root Directory is the repo root. Only the one at the
 * actual deployment root activates — the other is an inert source file.
 *
 ...
 *
 * Dependency-free on purpose: the "continue" response below is exactly what
 * @vercel/edge's next() returns, so no import is needed.
 */
